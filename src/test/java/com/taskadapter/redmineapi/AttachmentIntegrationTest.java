...
public class AttachmentIntegrationTest {
    ...
    public static void oneTimeSetup() {
        ...
        final Project project = IntegrationTestHelper.createProject(mgr);
    ...
    public void addAttachment() throws RedmineException, IOException {
        final File tempFile = File.createTempFile("redmine_test_", ".tmp");
        final FileWriter fileWriter = new FileWriter(tempFile.getAbsolutePath());
        final String attachmentContent = "some text";
        ...
        try {
            final Issue loadedIssue = issueManager.getIssueById(createdIssue.getId(), Include.attachments);
            final Collection<Attachment> attachments = loadedIssue.getAttachments();
            final Attachment next = attachments.iterator().next();
            ...
            final String contentAsString = new String(receivedContent);
    ...
    public void testDownloadAttachmentContent() throws RedmineException {
        ...
        // not support that.
        final int attachmentID = 1;
        // retrieve issue attachment
        final Attachment attachment = attachmentManager.getAttachmentById(attachmentID);
        // download attachment content
        final byte[] attachmentContent = attachmentManager.downloadAttachmentContent(attachment);
    ...
    public void testGetIssueWithAttachments() throws RedmineException {
        ...
        try {
            ...
            final Issue issueToCreate = IssueFactory.create(projectId, "testGetIssueAttachment_" + UUID.randomUUID());
            ...
            final Issue retrievedIssue = issueManager.getIssueById(newIssue.getId(),
                    Include.attachments);
    ...
    public void testUploadException() throws RedmineException, IOException {
        ...
        attachmentManager.uploadAttachment("test.bin", "application/ternary", content, new File("test.bin").length());
    }
    ...
    public void uploadAttachment() throws RedmineException, IOException {
        ...
        try {
            ...
            final Issue issueById = issueManager.getIssueById(createdIssue.getId(), Include.attachments);
